...
#ifndef LIB_WIZNET_SPI_DEVICE
#error "please config SPI device name in menuconfig."
#endif

#define DBG_ENABLE
#define DBG_SECTION_NAME               "wiz"
#ifdef LIB_WIZNET_DEBUG
...
//轮询线程
static void wiz_data_poll_thread_entry(void *parameter)
{
#define IR_SOCK(ch)         (0x01 << ch)   /**< check socket interrupt */
...
#ifndef LIB_WIZNET_USING_DHCP
    if(wiz_netstr_to_array(LIB_WIZNET_IPADDR, wiz_net_info.ip) != RT_EOK ||
            wiz_netstr_to_array(LIB_WIZNET_MSKADDR, wiz_net_info.sn) != RT_EOK ||
            wiz_netstr_to_array(LIB_WIZNET_GWADDR, wiz_net_info.dns) != RT_EOK ||
            wiz_netstr_to_array(LIB_WIZNET_GWADDR, wiz_net_info.gw) != RT_EOK)
    {
...
//    return 0;
//}
static int wiz_data_poll_init()
{
    /* create WIZnet SPI RX thread  */
    rt_thread_t tid = rt_thread_create("wiz", wiz_data_poll_thread_entry, RT_NULL, 1024, RT_THREAD_PRIORITY_MAX / 6, 20);
    if (tid != RT_NULL)
    {
        rt_thread_startup(tid);
    }
...
//    rt_pin_mode(WIZ_RST_PIN, PIN_MODE_OUTPUT);
    ...
    result = wiz_device_init(LIB_WIZNET_SPI_DEVICE);
    if (result != RT_EOK)
    {
        goto __exit;
    }
